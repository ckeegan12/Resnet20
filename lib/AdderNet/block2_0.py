...
class ResidualBlock2_0(nn.Module):
  ...
  """
  def __init__(self, in_channels, out_channels, bits, max_val=2.5, kernel_size=3, stride=1, padding=1, downsample=None):
    super(ResidualBlock2_0, self).__init__()
    self.adder1 = adder2d2_0(in_channels, out_channels, kernel_size, bits, max_val, stride, padding, bias=False)
    self.bn1 = BatchNorm2dWithAdderBias(out_channels, bits, max_val=max_val)
    self.relu = nn.ReLU()
    self.adder2 = adder2d2_0(out_channels, out_channels, kernel_size, bits, max_val, stride=1, padding=padding, bias=False)
    self.bn2 = BatchNorm2dWithAdderBias(out_channels, bits, max_val=max_val)
