...
class ResidualBlock(nn.Module):
  ...
  def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, downsample=None):
    ...
    self.adder1 = adder2d(in_channels, out_channels, kernel_size, stride, padding, bias=False)
    self.bn1 = nn.BatchNorm2d(out_channels)
    self.relu = nn.ReLU()
    self.adder2 = adder2d(out_channels, out_channels, kernel_size, stride=1, padding=padding, bias=False)
